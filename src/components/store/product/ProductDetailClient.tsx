...
export function ProductDetailClient({
  ...
  return (
    <div className="mt-6 border-t border-zinc-200 pt-4">
      ...
      <div className="grid gap-2 border-b border-zinc-200 pb-2 text-xs uppercase tracking-wide text-zinc-500 md:grid-cols-4">
        <div className="text-sm font-semibold text-zinc-700">Min QTY</div>
        <div className="text-sm font-semibold text-zinc-700">Max QTY</div>
        <div className="text-sm font-semibold text-zinc-700">Unit Price</div>
        <div className="text-sm font-semibold text-zinc-700">Quantity</div>
      </div>
      <div className="mt-2 grid items-center gap-3 rounded-lg bg-white py-2 md:grid-cols-4">
        <div className="text-sm text-zinc-700">1</div>
        <div className={`text-sm ${isOutOfStock ? "font-semibold text-rose-600" : "text-zinc-700"}`}>{isOutOfStock ? "0 (out of stock)" : product.stock}</div>
        <div className="text-sm font-semibold text-zinc-900">₹ {product.sellingPrice ?? product.price}</div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => updateQty(qty - 1)} className="h-6 w-6 rounded-sm border border-zinc-300 bg-brand-yellow text-sm font-semibold">
            -
          </button>
          <input
            value={qty}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          ...
          style={buttonStyle}
          className="rounded-sm bg-[var(--pdp-btn-bg)] px-5 py-2 text-sm font-semibold text-zinc-900 transition-colors hover:bg-[var(--pdp-btn-hover-bg)] disabled:opacity-60"
        >
          Add to cart
        </button>
        <button
