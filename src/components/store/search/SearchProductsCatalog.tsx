...
interface SearchProductsCatalogProps {
  query: string;
  categorySlug: string;
  categoryName: string;
  products: Product[];
...
export function SearchProductsCatalog({
  query,
  categorySlug,
  categoryName,
  products,
  ...
  actionButtonBg,
  actionButtonHoverBg,
}: SearchProductsCatalogProps) {
  const hasQuery = Boolean(query.trim());
  const hasCategoryFilter = Boolean(categorySlug && categorySlug !== "all");
  const title = hasQuery ? `Search results for "${query}"` : "Browse products";
    ...
    <section className="rounded border border-zinc-200 bg-white">
      <div className="border-b border-zinc-200 px-4 py-3 md:px-6">
        <nav className="text-xs text-zinc-500" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center gap-1.5">
            ...
            <li className="font-medium text-zinc-700">Search</li>
          </ol>
        </nav>
        <h1 className="mt-3 text-2xl font-semibold tracking-tight text-zinc-900 md:text-3xl">{title}</h1>
        <p className="mt-1 text-sm text-zinc-500">
          {products.length} product{products.length === 1 ? "" : "s"} found
          {hasCategoryFilter ? ` in ${categoryName}` : ""}
        </p>
      ...
      {products.length ? (
        <div className="p-3 md:p-4">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-2.5 md:grid-cols-3 md:gap-3 lg:grid-cols-4 xl:grid-cols-4">
                    ...
                    <div className="flex flex-1 flex-col px-2 pb-2 pt-1.5 sm:px-2.5 sm:pb-2.5 sm:pt-2">
                      ...
                      <div className="mt-auto pt-1.5">
                        {product.stock > 0 && product.stock < 5 ? (
                          ...
                          <p className="text-[11px] text-zinc-500">Category: {(product.categorySlug || categorySlug || "all").toUpperCase()}</p>
                        )}
                      </div>
                    </div>
                  </Link>
        ...
        <div className="px-4 py-10 text-center md:px-6">
          <p className="text-base font-medium text-zinc-700">
            {hasQuery ? `No products matched "${query}".` : "No products available right now."}
          </p>
          <p className="mt-1 text-sm text-zinc-500">Try another keyword, remove filters, or browse all categories.</p>
          <Link
            href="/category/all"
            className="mt-4 inline-flex rounded-full bg-[var(--search-btn-bg)] px-5 py-2 text-sm font-semibold text-zinc-900 transition hover:bg-[var(--search-btn-hover-bg)]"
            style={
              {
                "--search-btn-bg": actionButtonBg,
                "--search-btn-hover-bg": actionButtonHoverBg,
              } as CSSProperties
            }
          >
            View all categories
