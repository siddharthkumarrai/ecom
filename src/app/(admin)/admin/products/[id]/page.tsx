...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!productRes.ok) {
        const body = (await productRes.json()) as { error?: string };
        setError(body.error || "Failed to load product.");
        setLoading(false);
        return;
      }

      const productJson = (await productRes.json()) as { item?: ProductItem };
      ...
      setForm({
        ...
        imageUrlsText: (item.images ?? []).join("\n"),
        categoryId,
        brandId,
        costPrice: String(typeof item.costPrice === "number" ? item.costPrice : 0),
        sellingPrice: String(item.basePrice ?? 0),
        productDeliveryCharge: String(item.productDeliveryCharge ?? 0),
        stock: String(item.stock ?? 0),
        isActive: !!item.isActive,
      });
      const initialSpecifications =
        item.specifications?.map((spec) => ({ key: spec.key ?? "", value: spec.value ?? "" })).filter((spec) => spec.key || spec.value) ?? [];
      const initialDocuments =
        item.technicalDocuments?.map((doc) => ({ name: doc.name ?? "", url: doc.url ?? "" })).filter((doc) => doc.name || doc.url) ?? [];
      setSpecifications(initialSpecifications.length ? initialSpecifications : [{ key: "", value: "" }]);
      setTechnicalDocuments(initialDocuments.length ? initialDocuments : [{ name: "", url: "" }]);
      setLoading(false);
    };
    run();
  }, [id]);
  ...
  const submit = async () => {
    ...
    if (!res.ok) {
      ...
      const firstFieldError = body.details?.fieldErrors ? Object.values(body.details.fieldErrors).flat().find(Boolean) : "";
      setError(firstFieldError || body.error || "Failed to update product");
      return;
    }
  ...
  const onUploadImage = async (file: File) => {
    ...
    try {
      ...
      setForm((prev) => {
        const lines = prev.imageUrlsText
          .split("\n")
          .map((line) => line.trim())
          .filter(Boolean);
  ...
  const removeImage = async () => {
    const current = imageLines[0];
    if (!current) return;
    await fetch("/api/v1/admin/products", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageUrl: current }) });
    setForm((prev) => ({
      ...prev,
      imageUrlsText: prev.imageUrlsText.split("\n").map((line) => line.trim()).filter((line) => line && line !== current).join("\n"),
    }));
  };
    ...
    return (
      <section className="max-w-3xl space-y-4">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">Loading product details...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="max-w-3xl space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={() => router.push("/admin/products")} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
          Cancel
        </button>
      </div>
    </section>
